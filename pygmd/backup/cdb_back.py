...
def Incarmethod(incarstring):
    incar = {"NSW":500,"LCHARG":False,"ISMEAR":0,"EDIFF":1E-6,"ICHARG":0}
...
class PerovInputs :
    ...
    def _poscar(self, delete_selective=False):
        # CIF has charge, so charge delete
        if delete_selective :
            for i in range(self.poscar.num_sites):
                try :
    ...
    def _incarmode(incar, method):
        if method == "R" :
            ...
            incar["LCHARG"] = True
            incar["EDIFF"] = 1E-6
        elif method == "B" or method == "E" or method == "D" or method == "H":
            incar["EDIFF"]=1E-6
            incar["NSW"]=0
            incar["LCHARG"] = False
            incar["ICHARG"] = 11
