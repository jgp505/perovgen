...
class DOSPlotting :
	def __init__(self, vasprun="vasprun.xml",dos=None, zero_to_efermi=True, stack=True) :
		...
		self.stack = stack
		self.efermi = Vasprun(vasprun).efermi
...
class BSPlotting :
    ...
    def _bandinform(self):
        ...
        # Fermi energy & band gap 
        band_inform['E_f'] = self.bs.efermi
        band_inform['E_g'] = self.bsdict['band_gap']['energy']
        ...
        for index in cbm["kpoint_index"] :
            cbm1.append((self.bs.distance[index],cbm['energy']))
        for index in vbm['kpoint_index'] :
            vbm1.append((self.bs.distance[index],vbm['energy']))
                
        band_inform['CBM'] = cbm1 
        band_inform['VBM'] = vbm1 
