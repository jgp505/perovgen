...
class GMDPhase :
    def __init__ (self, vasprun, PBEmode=True) :
        self.mpr = MPRester()
        self.vrun = Vasprun(vasprun)
        self.pbe = PBEmode
        entry = self.vrun.get_computed_entry(inc_structure=True)
        element = [e.symbol for e in self.vrun.final_structure.composition.elements]
        self.entries = self.mpr.get_entries_in_chemsys(element)
        if self.pbe :
            MPC = MaterialsProjectCompatibility()
            entry = MPC.process_entry(entry)
            self.entries = MPC.process_entries([entry] + self.entries)
        self.pd = PhaseDiagram(self.entries)

    def formation_and_hull_energy(self):
        # Make PhaseDiagram
        decomp, ehull = self.pd.get_decomp_and_e_above_hull(self.entries[0])
        formation = self.pd.get_form_energy_per_atom(self.entries[0])
        return ehull, formation

    def PhaseDiagramPlotter(self, hulle) :
        plotter = PDPlotter(self.pd, show_unstable=hulle)
    ...
    def make_csv(self) :
        ...
        for e in self.entries : 
            decomp, ehull = self.pd.get_decomp_and_e_above_hull(e)
            if e.entry_id == None :
                data["Materials ID"].append("GMD-{}".format(number))
                number += 1
            else :
                data["Materials ID"].append(e.entry_id)
            data["Composition"].append(e.composition.reduced_formula)
            data["Hull"].append(ehull)
            data["Formation"].append(self.pd.get_form_energy_per_atom(e))
            data["Decomposition"].append(" + ".join(["%.2f %s" % (v, k.composition.formula) for k, v in decomp.items()]))

        df = panda.DataFrame(data, columns=["Materials ID","Composition","Hull","Formation","Decomposition"])
        return df
