...
for k,v in data.items():
    ...
    for v1 in v :
        if v1 == "C_mode" :
            ...
            dataframe["Symmetry"].append(space)
            dataframe["SPN"].append(number)
            ...
            element = list(set(s.species))
            oxistate = s.composition.oxi_state_guesses()[0]
            if Element("H") and Element("C") and Element("N") in element :
                sn = s.composition.get_el_amt_dict()
                a = GMDStructure(s)._split_molecule()
                dataframe['Asite'].extend(list(a.keys()))
                dataframe['A_num'].extend(list(a.values()))
                dataframe["Asite_oxistate"].append(1)
                del sn['H']
                del sn['C']
                del sn['N']
                e = 0
                for ks, vs in sn.items() :
                    if e == 0 :
                        dataframe['Bsite'].append(ks)
                        dataframe['B_num'].append(vs)
                        dataframe["Bsite_oxistate"].append(oxistate[ks])
                        e+= 1
                    else :
                        dataframe['Xsite'].append(ks)
                        dataframe['X_num'].append(vs)
                        dataframe["Xsite_oxistate"].append(1)
            else :
            ...
            em_e = open("{}/{}/EM".format(k,v1),'r',encoding='utf-8').readlines()[-12:]
    ...
    df = pd.DataFrame(dataframe,columns=["path","Formula","Asite","Bsite","Xsite","A_num","B_num","X_num","Asite_oxistate","Bsite_oxistate",
                                        "Xsite_oxistate","W","total_electrons","electronegativity",
                                        "a","b","c","alpha","beta","gamma","volume","density","Symmetry","SPN","Energy","E_hull","E_form",
                                        "Ef","Eg","Eg_indirect","Eg_direct","CBM_Kindex","CBM_Bindex","VBM_Kindex","VBM_Bindex","me*","mh*"],dtype=object)
