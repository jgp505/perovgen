...
from perovgen.pygmd.base import load_structure, GMDStructure, pdos
...
def Process(inputs, strucpath, ds=False, orbit=False):
    ...
    for mt in inputs["METHOD"] :
        ...
        for i in runfolder :
            i = os.path.abspath(i)
            if mt == "C" or mt == "R" :
                if inputs["INCAR"][0] == "MPJ" or inputs["INCAR"][0] == "PBE" :
                    files = ["{}/vasprun.xml".format(i)]
                    entries, pd = GMDPhaseDiagram(files,extractcomp=None).get_phasediagram()
                    df = GMDPhaseDiagram.get_data(entries, pd)
                    df.to_csv("{}/{}_Phase.csv".format(i,i.split("/")[-1]))
                else :
                    pass
            elif mt == "B" :
                bsp = BSPlotting(vasprun=os.path.abspath("{}/vasprun.xml".format(i)), kpoints=os.path.abspath("{}/KPOINTS".format(i)))
                bsp.printinform()
                fig = plt.figure(figsize=(12,8))
                bsp.get_plot().savefig("{}/{}.pdf".format(i,i.split("/")[-1]))
                ...
                name, number, mode = path.split("_")[0:-1]

                # effective mass
                em_e = open("{}/{}_{}_E_mode/EM".format(pwd,name,number),'r').readlines()[-12:]
                em_h = open("{}/{}_{}_H_mode/EM".format(pwd,name,number)).readlines()[-12:]
                E, H = GMDExcitonbinding.harm_mean_em(em_e), GMDExcitonbinding.harm_mean_em(em_h)

                # Outcar
                outcar = open("{}/{}_{}_U_mode/OUTCAR".format(pwd,name,number)).readlines()
                dielec = GMDExcitonbinding.dielectricconst(outcar)
                print("Dielectric Const : ", dielec)

                # reduced effecitve mass
                mu = (E*H)/(E+H)
                exciton = (mu*13.605692)/(dielec**2)
                print("Exciton binding Energy is : ",exciton*1E+3,"(meV)")
...
def AutoMolOpt(strucpath, inputs) :
    ...
    for path in strucpath : 
        ...
        energy = [Vasprun(p1).final_energy for p1 in path1] 
