			...
			for (int isp=0; isp<ispin; isp++) {
				for (int ikp=0; ikp<nkpts; ikp++) {
					...
					for (int i=0; i<nbands; i++) {
						fscanf(ifp, "%d%lf%lf", &idum, &band_en[iev],
								&band_occ[iev]);
						band_kvec[iev] = ikp;
						printf("ikp=%d, iband=%d, band_en=%lf\n",ikp,i,band_en[iev]);
						iev++;
					}
				}
			}
		...
		for (int j=0; j<pdos_list[i].ions.size(); j++) {
			...
			for (int is=0; is<ispin; is++) {
				if (isp != 2 && is != isp)
					continue;
				for (int k=0; k<nkpts; k++) {
					int ik = is * nkbi110 + k * nbands * ndata * 10;
					for (int b=0; b<nbands; b++) {
						int ib = ik + b * ndata * 10;
						int i1 = ib + (iion - 1) * 10 + iorbit;
						int i2 = ib + (ndata - 1) * 10 + itot;
						if (orbital_wgt[i2] < 1e-10) {
							fprintf(stderr, 
									"\nk-point %d band %d has incorrect PDOS data: "
									"the total projection weight is zero. exit.\n",
									k+1, b+1);
							exit(-1);
						}
						double wgt = orbital_wgt[i1] / orbital_wgt[i2];
	...
	for (int i=0; i<phase_list.size(); i++) {
		...
		for (int j=0; j<phase_list[i].kvec.size(); j++) {
			...
			for (int is=0; is<ispin; is++) {
				...
				int ik = is * nkbi110 + (k - 1) * nbands * ndata * 9;
				int ib = ik + (iband - 1)* ndata * 9;
				for (int iion=0; iion<nions; iion++) {
					int l = ib + iion * 9 + iorbit;
					double rho, theta;
...
void PDOS::read_PROCAR()
{
	...
	orbital_wgt.resize(nkpts * nbands * ndata * 10 * 2); /* 10 orbitals, 2 spins */
	kvec.resize(nkpts);
	wf_real.resize(nkpts * nbands * ndata * 9 * 2); /* 9 orbitals, 2 spins */
	wf_imag.resize(nkpts * nbands * ndata * 9 * 2); /* 9 orbitals, 2 spins */
...
{
	int ndata = (nions == 1) ? 1 : nions + 1;
	int indx = nkpts * nbands * ndata * 10 * (ispin - 1);
	int indx1 = nkpts * nbands * nions * 9 * (ispin - 1);
	int indx2 = nkpts * nbands * nions * 9 * (ispin - 1);
	char str[256];
	for (int k=0; k<nkpts; k++) {
		char* strptr;
		...
		valarray<double>& kvec_wgt)
{
    ...
    full_dos[0] = 3*full_dos[1]-3*full_dos[2]+full_dos[3];  // exterapolation
